...
from odlc_classify_shape import (
    cart2pol,
    toPolar,
    merge_sort,
    condense_polar,
    Generate_Polar_Array,
    Verify_Shape_Choice,
    Compare_Based_On_Peaks,
    classify_shape,
    Image_Address_To_Contour,
    classify_shape,
)
...
class TestShapeClassification(unittest.TestCase):
    ...
    def test_classify_shape(self):
        ...
        for i in range(8):
            contour_sample = Image_Address_To_Contour(
                "vision/unit_tests/test_images/shape" + str(i + 1) + ".png"
            )
